...
The Aiblockchain Core daemon can be configured to act as a trusted "border
router", implementing the aiblockchain wire protocol and relay, making
...
The ZeroMQ feature in Aiblockchain Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building aiblockchaind:
...
For instance:
    ...
    $ aiblockchaind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/aiblockchaind.tx.raw \
...
These options can also be provided in aiblockchain.conf.
...
From the perspective of aiblockchaind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into aiblockchaind directly. Furthermore, no information is
...
using. Aiblockchaind appends an up-counting sequence number to each
