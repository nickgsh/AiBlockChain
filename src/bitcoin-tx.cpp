...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " aiblockchain-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  aiblockchain-tx [options] <hex-tx> [commands]  Update hex-encoded aiblockchain transaction\n" +
            "or:     aiblockchain-tx [options] -create [commands]   Create hex-encoded aiblockchain transaction\n" +
            "\n";
